 ...
 */
app.post('/api/workflows/:workflowId/generate-draft', async (req: Request, res: Response) => {
  try {
    ...
    const result = await workflow.generateEmailDraft(getContext(), workflowId, campaign);
...
app.post('/api/emails/:emailId/send', async (req: Request, res: Response) => {
  try {
    ...
    const result = await workflow.sendEmailWithApproval(getContext(), emailId, approval);
