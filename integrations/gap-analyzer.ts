...
export class GapAnalyzer {
  ...
  analyzeGaps(
    ...
    const missing_information = this.identifyMissingInfo(text, entities, intent);
    const suggested_additions = this.generateSuggestions(text, entities, intent, tone);
    const completeness_score = this.calculateCompleteness(text, entities, intent);
  ...
  private identifyMissingInfo(
    ...
    switch (intent.primaryGoal) {
      case 'funding':
        if (!lowerText.includes('budget') && !lowerText.includes('amount') && !lowerText.includes('$')) {
          missing.push('Specific funding amount or budget range');
        }
        if (!lowerText.includes('purpose') && !lowerText.includes('use of funds')) {
      ...
      case 'partnership':
        ...
        if (!lowerText.includes('role') && !lowerText.includes('responsibility')) {
          missing.push('Clear definition of partnership roles');
        }
        break;

      case 'volunteers':
        if (!lowerText.includes('number') && !lowerText.includes('how many')) {
          missing.push('Number of volunteers needed');
        }
        if (!lowerText.includes('skills') && !lowerText.includes('experience')) {
          missing.push('Required skills or experience');
        }
        if (!lowerText.includes('time') && !lowerText.includes('duration')) {
          missing.push('Time commitment required');
        }
        break;

      case 'advocacy':
        if (!lowerText.includes('policy') && !lowerText.includes('legislation')) {
          missing.push('Specific policy or legislative goals');
        }
        if (!lowerText.includes('target') && !lowerText.includes('audience')) {
          missing.push('Target audience for advocacy');
        }
        break;

      case 'research':
        if (!lowerText.includes('methodology') && !lowerText.includes('approach')) {
          missing.push('Research methodology or approach');
        }
        if (!lowerText.includes('outcome') && !lowerText.includes('deliverable')) {
          missing.push('Expected research outcomes');
        }
        break;
    }

    // Geographic scope missing
    if (intent.scope === 'regional' && entities.locations.length === 0) {
      missing.push('Specific geographic location or region');
    }

    // Timeline missing
    if (intent.timeline === 'short-term' && !lowerText.includes('deadline') && !lowerText.includes('date')) {
      missing.push('Specific timeline or deadline');
    }

    // Contact information missing
    if (!lowerText.includes('email') && !lowerText.includes('contact') && !lowerText.includes('phone')) {
  ...
  private generateSuggestions(
    ...
    if (entities.causes.length === 0) {
      suggestions.push('Specify the environmental or social cause you\'re addressing');
    }
    ...
    if (intent.primaryGoal === 'volunteers') {
      suggestions.push('Clearly specify skills needed and time commitment');
    }
    ...
    if (sentences.length < 3) {
      suggestions.push('Structure your brief with multiple sentences for clarity');
    }
  ...
  private calculateCompleteness(
    ...
    switch (intent.primaryGoal) {
      case 'funding':
        if (text.toLowerCase().includes('budget') || text.toLowerCase().includes('$')) score += 10;
        break;
      case 'volunteers':
        if (text.toLowerCase().includes('skills') || text.toLowerCase().includes('time')) score += 10;
        break;
      case 'partnership':
  ...
  private calculateClarity(
    ...
    // Avoid vagueness
    const vagueWords = ['help', 'support', 'work', 'things', 'stuff', 'something'];
    const lowerText = text.toLowerCase();
  ...
  private identifyPriorityImprovements(
    ...
    const highPriorityMissing = missing.filter(m => 
      m.includes('funding') || m.includes('timeline') || m.includes('contact')
    );
    priorities.push(...highPriorityMissing);

    // Priority suggestions
    const highPrioritySuggestions = suggestions.filter(s => 
      s.includes('specific') || s.includes('measurable') || s.includes('clear')
    );
