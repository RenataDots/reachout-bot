...
import { IntentAnalyzer, BriefIntent } from "./intent-analyzer";
import { EntityExtractor, BriefEntities } from "./entity-extractor";
import { ToneAnalyzer, BriefTone } from "./tone-analyzer";
import { GapAnalyzer, BriefGaps } from "./gap-analyzer";
...
export class BriefProcessor {
  ...
  constructor(
    ...
    this.intentAnalyzer = new IntentAnalyzer(this.logger);
    this.entityExtractor = new EntityExtractor(this.logger);
    this.toneAnalyzer = new ToneAnalyzer(this.logger);
    this.gapAnalyzer = new GapAnalyzer(this.logger);
  }
  ...
  async processBrief(text: string): Promise<ProcessedBrief> {
    ...
    this.logger(
      `[BriefProcessor] Brief processed: ${result.wordCount} words, quality score: ${quality.score}`,
    );
    this.logger(
      `[BriefProcessor] Phase 2 complete - Intent: ${intent.primaryGoal}, Confidence: ${intent.confidence}%`,
    );
    return result;
  }
  ...
  private cleanText(text: string): string {
    ...
    this.logger(
      `[BriefProcessor] Text cleaned: ${text.length} → ${cleaned.length} characters`,
    );
    return cleaned;
  }
  ...
  private detectStructure(text: string): TextSegment[] {
    ...
    this.logger(
      `[BriefProcessor] Structure detected: ${segments.length} segments`,
    );
    return segments;
  }
  ...
  private assessTextQuality(
    text: string,
    structure: ProcessedBrief["structure"],
  ): ProcessedBrief["quality"] {
    ...
    if (wordCount < 10) {
      ...
      suggestions.push(
        "Consider focusing on key points for better readability",
      );
      score -= 10;
    }
    ...
    if (structure.estimatedFormat === "unknown") {
      ...
      suggestions.push(
        "Use paragraphs or bullet points for better organization",
      );
      score -= 20;
    } else if (
      structure.estimatedFormat === "paragraph" &&
      !structure.hasMultipleParagraphs
    ) {
      issues.push("Single paragraph may be hard to read");
      suggestions.push(
        "Consider breaking into multiple paragraphs or using bullet points",
      );
      score -= 10;
    }
    ...
    if (!this.containsKeyInformation(text)) {
      ...
      suggestions.push(
        "Include details about budget, timeline, or specific objectives",
      );
      score -= 25;
    }
    ...
    if (!this.hasActionableContent(text)) {
      ...
      suggestions.push(
        "Add specific needs or what you're looking for from partners",
      );
      score -= 15;
    }
    ...
    this.logger(
      `[BriefProcessor] Quality assessment: ${score}/100, ${issues.length} issues`,
    );
