...
We'd love to explore potential synergies and ways we might work together.`,
  ...
  const emailResult = await workflow.generateEmailDraft(
    ctx,
    workflowId,
    campaign,
  );
  ...
  logger("", "info");
  logger(
    "Step 4: [USER REVIEW] Email is presented to human for approval",
    "info",
  );
  ...
  const sendResult = await workflow.sendEmailWithApproval(
    ctx,
    draftEmail.id,
    approval,
  );
  ...
  const assessment = await workflow.getAdvisoryRiskAssessment(
    ctx,
    ngoProfile.id,
  );
  ...
  logger(
    `Note: Risk score never blocks outreach - for human review only`,
    "info",
  );
  ...
  logger(
    `- Mock data directory: ${process.env.MOCK_DATA_DIR || "./data/mock-data"}`,
    "info",
  );
  logger("- Check data/mock-data/sent-email-*.json for email details", "info");
  logger("- Check data/mock-data/mock-database.json for full state", "info");
}
